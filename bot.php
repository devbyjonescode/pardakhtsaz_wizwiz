...
if(preg_match('/^changePaymentKeys(\w+)/',$data,$match)){
    ...
    switch($match[1]){
        ...
        case "tronwallet":
            ...
            break;
        case "pardakhtSazId":
            $gate = "مقدار secret id پرداخت ساز";
            break;
        case "pardakhtSazKey":
            $gate = "مقدار secret key پرداخت ساز";
            break;
        case "pardakhtSazWallet":
            $gate = "آدرس ولت پرداخت ساز";
            break;
        case "pardakhtSazUnitId":
            $gate = "آی دی واحد ولت پرداخت ساز";
            break;
    }
...
if($userInfo['step'] == "increaseMyWallet" && $text != $buttonValues['cancel']){
    ...
    if($botState['pardakhtSazState'] == "on"){
        $keyboard[] = [['text' => $buttonValues['pardakht_saz'],  'callback_data' => "payWithPardakhtSaz$hash_id"]];
    }
...
if(preg_match('/payWithPardakhtSaz(.*)/',$data,$match)) {
    ...
    $payResponse = guestPay($payInfo['price'],$payInfo['user_id']);


    if(isset($payResponse->data->errorCode) && $payResponse->data->errorCode == 0){
        $payResponseData = $payResponse->data;
        if(isset($payResponseData->content->reference)){
            $stmt = $connection->prepare("UPDATE `pays` SET `reference` = ? WHERE `hash_id` = ?");
            $stmt->bind_param("ss", $payResponseData->content->reference, $match[1]);
            $stmt->execute();
            $stmt->close();
        }
...
",$keys);
        ...
        $errorList = [
            "100001" => "کلیدهای ورودی نامعتبر میباشند.",
            "100002" => "اعتبارسنجی مقادیر ورودی با خطا مواجه شده است.",
            "100003" => "شناسه IP نامعتبر میابشد.",
            "100004" => "شماره موبایل نامعتبر میباشد.",
            "100005" => "به دلیل درخواست زیاد، ارسال پیامک برای کاربر محدود شده است.",
            "100006" => "کد یکبار مصرف قبلی هنوز معتبر میباشد.",
            "100007" => "ارسال پیامک با خطا مواجه شده است.",
            "100008" => "کد یکبار مصرف با شماره موبایل همخوانی ندارد.",
            "100009" => "نقش کاربر در سیستم وارد نشده است.",
            "100010" => "کاربر غیر فعال می باشد.",
            "100011" => "کاربر یافت نشد.",
            "100012" => "دسترسی لازم وجود ندارد.",
            "100013" => "کلید های امنیتی قبلا ایجاد شده است.",
            "100014" => "رمز عبور قبلا ایجاد شده است.",
            "100015" => "رمز عبور به درستی از سوی کاربر تایید نشده است.",
            "100016" => "فرمت رمزعبور صحیح نمی باشد.",
            "100017" => "شماره موبایل کاربر تایید نشده است.",
            "100018" => "شماره موبایل یا رمز عبور وجود ندارد.",
            "100019" => "وب هوک قبلا ثبت شده است.",
            "100020" => "پاسخی از وب هوک دریافت نشد.",
            "100021" => "کلید ارتباط داخلی معتبر نمیباشد.",
            "100022" => "کابر دسترسی لازم را ندارد.",
            "100023" => "ایجاد کیف پول مبتنی بر شبکه ترون با مشکل مواجه شده است.",
            "100024" => "شبکه مورد نظر موجود نمی باشد.",
            "100025" => "کاربر دارای کیف پول فعال میباشد.",
            "100026" => "ساخت ماربر جدید به دلیل عدم ایجاد کیف پول رمزارزی ممکن نمیباشد.",
            "100027" => "نوع سفارش معتبر نمی باشد.",
            "100028" => "هیچ درگاهی یافت نشد.",
            "100029" => "هیچ واحد ارزی یافت نشد.",
            "100030" => "هیچ شبکه پرداختی یافت نشد.",
            "100031" => "هیچ دسته بندی یافت نشد.",
            "100032" => "هیچ وضعیت پرداختی یافت نشد.",
            "100033" => "وضعیت پرداختی مورد نظر یافت نشد.",
            "100034" => "دسته بندی مورد نظر یافت نشد.",
            "100035" => "شبکه پرداخت مورد نظر یافت نشد.",
            "100036" => "واحد ارزی مورد نظر یافت نشد.",
            "100037" => "درگاه مورد نظر یافت نشد.",
            "100038" => "هیج درگاهی برای ارز وارد شده یافت نشد.",
            "100039" => "وضعیت هویتی کاربر یافت نشد.",
            "100040" => "هیچ درگاه پرداخت ریالی و فعالی یافت نشد.",
            "100041" => "هیچ قیمت فعالی برای دلار یافت نشد.",
            "100042" => "کیف پول مورد نظر یافت نشد.",
            "100043" => "کلید های امنیتی معتبر نمی باشد.",
            "100044" => "امکان ثبت سفارش خرید ریال ممکن نمی باشد.",
            "100045" => "مبلغ فاکتور بیش از حد مجاز می باشد.",
            "100046" => "مبلغ ریالی مورد نظر خارج از باره مجاز درگاه پرداخت می باشد.",
            "100047" => "وضعیت سفارش یافت نشد.",
            "100048" => "شناسه سفارش معتبر نمیباشد.",
            "100049" => "سفارش مورد نظر یافت نشد.",
            "100050" => "سفارش قبلا تایید شده است.",
            "100051" => "وضیعیت درانتظار برای سفارش یافت نشد.",
            "100052" => "بروزرسانی متادیتا با مشکل مواجه شده است.",
            "100053" => "وضیعیت باطل شده برای سفارش یافت نشد.",
            "100054" => "به دلیل غیر فعال بودن واحد، سفارش باطل گردید.",
            "100055" => "به دلیل غیر فعال بودن درگاه، سفارش باطل گردید.",
            "100056" => "شماره حساب وارد شده قبلا ثبت گردیده است.",
            "100057" => "شماره شماره کارت وارد شده قبلا ثبت گردیده است.",
            "100058" => "شماره شماره شبا وارد شده قبلا ثبت گردیده است.",
            "100059" => "شماره کارت وارد شده معتبر نمی باشد.",
            "100060" => "شماره شبا وارد شده معتبر نمی باشد.",
            "100061" => "اطلاعات اساسی مرتبط با حساب بانکی معتبر نمیباشد.",
            "100062" => "شناسه IP وارد شده صحیح نمی باشد.",
            "100063" => "تراکنش خارجی یافت نشد.",
            "100064" => "وضیعیت پرداخت شده برای تراکنش یافت نشد.",
            "100065" => "تراکنش خارجی در وضعیت مجاز نمیباشد.",
            "100066" => "مبلغ وارد شده نسبت به تراکنش خارجی مغایرت دارد.",
            "100067" => "وضیعیت اولیه برای تراکنش یافت نشد.",
            "100068" => "تراکنش خارجی ارسال شده قابل ثبت نمیباشد.",
            "100069" => "وضیعیت فعلی با درخواست یکسان است.",
            "100070" => "کیف پول ریالی کاربر یافت نشد.",
            "100071" => "کیف پول ارزی کاربر یافت نشد.",
            "100072" => "نوع داده موجودی کیف پول کاربر معتبر نمیباشد.",
            "100073" => "مدل بروزرسانی کیف پول صحیح نمیباشد.",
            "100074" => "مبلغ نمیتواند صفر با کمتر از آن باشد.",
            "100075" => "مبلغ درخواستی از موجودی بیشتر است.",
            "100076" => "شناسه تراکنش متبر نمیباشد.",
            "100077" => "تراکنش داخلی مدنظر یافت نشد.",
            "100078" => "امکان بروزرسانی موجودی کیف پول به دلیل عدم پرداخت تراکنش داخلی مقدور نیمباشد.",
            "100079" => "افزایش موجودی با این شناسه تراکنش داخلی تکراری است.",
            "100080" => "تراکنش خارجی ارائه شده متعلق به این کاربر نمیباشد.",
            "100081" => "خطا در دریافت وضعیت سفارش.",
            "100082" => "سفارش در وضعیت مجاز جهت ادامه قرار ندارد.",
            "100083" => "وضیعیت منقضی شده برای سفارش یافت نشد.",
            "100084" => "به دلیل گذشتن زمان مجاز، سفارش منقضی گردید.",
            "100085" => "پاسخی از بانک دریافت نشد.",
            "100086" => "امکان ایجاد تراکنش ریالی با درگاه مدنظر ممکن نمیباشد.",
            "100087" => "امکان ایجاد پرداخت وجود ندارد.",
            "100088" => "ایجاد تراکنش خارجی با خطا مواجه شده است.",
            "100089" => "تراکنش خارجی با شناسه درگاه ارسالی وجود دارد.",
            "100090" => "وضعیت فعلی با وضعیت جدید یکسان است.",
            "100091" => "وضعیت فعلی قابل بروزرسانی نمیباشد.",
            "100092" => "وضیعیت باطل شده برای تراکنش یافت نشد.",
            "100093" => "به دلیل غیر فعال بودم وضعیت جاری، تراکنش باطل گردید.",
            "100094" => "تراکنش خارجی مورد نظر بروزرسانی نشد.",
            "100095" => "سفارش یافت نشد.",
            "100096" => "وضعیت درحال پرداخت یافت نشد.",
            "100097" => "به دلیل غیر فعال بودم وضعیت جاری، سفارش باطل گردید.",
            "100098" => "سفارش مورد نظر بروزرسانی نشد.",
            "100099" => "تراکنش خارجی با درگاه همخوانی ندارد.",
            "100100" => "وضعیت پرداخت دریافتی از درگاه معتبر نمی باشد.",
            "100101" => "بارگذاری فایل با مشکل مواجه شد.",
            "100102" => "کد ملی کاربر قبلا تایید شده است.",
            "100103" => "کد ملی معتبر نمیباشد.",
            "100104" => "تطبیق کد ملی و شماره موبایل در این لحظه مقدور نمیباشد.",
            "100105" => "کد ملی و شماره موبایل همخوانی ندارند.",
            "100106" => "کد ملی کاربر تایید نشده است.",
            "100107" => "این کد ملی قابل ثبت نمیباشد.",
            "100108" => "تایید تراکنش از سوی درگاه با خطا مواجه شده است.",
            "100109" => "تایید تراکنش قبلا انجام شده است.",
            "100110" => "وضیعیت در حال تایید برای تراکنش یافت نشد.",
            "100111" => "وضیعیت تایید شده برای تراکنش یافت نشد.",
            "100112" => "وضیعیت خرج شده برای تراکنش یافت نشد.",
            "100113" => "وضیعیت پرداخت شده برای تراکنش یافت نشد.",
            "100114" => "پارامترهای ارسالی نامعتبر میباشد.",
            "100115" => "موضوع ارسالی نامعتبر است.",
            "100116" => "شماره تلفن وارد شده معتبر نمیباشد.",
            "100117" => "کد پستی وارد شده صحیح نمیباشد.",
            "100118" => "امکان ثبت شماره تماس وارد شده وجود ندارد.",
            "100119" => "امکان ثبت کد پستی وارد شده وجود ندارد.",
            "100120" => "فراخوانی تصویر با خطا مواجه شده است.",
            "100121" => "تاریخ تولد مجاز به ثبت نمیباشد.",
            "100122" => "تاریخ تولد با کد ملی اعلامی تطابق ندارد.",
            "100123" => "تاریخ تولد تایید نشده است.",
            "100124" => "نام و نام خانادگی کاربر تایید نشده است.",
            "100125" => "نام و تاریخ تولد قبلا تایید شده است.",
            "100126" => "سفارش مورد نظر متعلق به این کاربر نمیباشد.",
            "100127" => "فراخوانی سفارش ها با مشکل مواجه شده است.",
            "100128" => "فرمت متادیتا معتبر نمیباشد.",
            "100129" => "کانال پرداختی معتبر نمیباشد.",
            "100130" => "ایمیل وارد شده معتبر نمیباشد.",
            "100131" => "متن پیام ارسالی معتبر نمیباشد.",
            "100132" => "نام یا نام خانوادگی معتبر نمیباشد.",
            "100133" => "نوع مقدار ورودی نامعتبر است.",
            "100134" => "مقدار ورودی استاندارد نمیباشد.",
            "100135" => "خطا ناشناخته ای رخ داده است.",
            "100136" => "فرمت مقادیر ورودی معتبر نمیباشند.",
            "100137" => "داده ورودی از نوع عددی نمیباشد.",
            "100138" => "داده ورودی از نوع رشته ای نمیباشد.",
            "100139" => "فایل پیوست نشده است.",
            "100140" => "پسوند فایل ارسالی معتبر نمیباشد.",
            "100141" => "فرمت تاریخ اشتباه است.",
            "100142" => "مقدار سال، ماه و یا روز معتبر نمیباشد.",
            "100143" => "تاریخ تول وارد شده در محدوده سنی مجاز قرار ندارد.",
            "100144" => "پارامتر ارسالی فاقد محتوا میباشد.",
            "100145" => "فرمت دریافتی از سامانه استعلام معتبر نمیباشد.",
            "100146" => "مقدار درخواستی در بازه مجاز نمیباشد.",
            "100147" => "کاربر دارای سطح کاربری مجاز نمیباشد.",
            "100148" => "شماره کارت وارد شده معتبر نمیباشد.",
            "100149" => "تاریخ تولد شمسی کاربر وجود ندارد.",
            "100150" => "کارت وارد شده متعلق به این کاربر نمیباشد.",
            "100151" => "مقدار فاقد محتوا است.",
            "100152" => "مقادیر بروزرسانی معتبر نمیباشد.",
            "100153" => "کلید های امنیتی موجود نمیباشد.",
            "100154" => "این ایمیل قبلا در خبرنامه ثبت شد است.",
            "100155" => "هیچ حساب بانکی برای کاربر یافت نشد.",
            "100156" => "مبلغ سفارش بیش از سقف مجاز کاربر میباشد.",
            "100157" => "کیف پول کاربر غیر فعال میباشد.",
            "100158" => "تراکنش مورد نظر یافت نشد.",
            "100159" => "شناسه درگاه معتبر نمیباشد.",
            "100160" => "ثبت تراکنش داخلی با خطا مواجه شده است.",
            "100161" => "وضعیت پرداخت برداشت شده یافت نشد.",
            "100162" => "پارامترهای ارسالی درگاه پرداخت نامعتبر میباشند.",
            "100163" => "فراخوانی پارانتر های ارسالی درگاه پرداخت با خطا مواجه شده است.",
            "100164" => "پرداخت از سوی درگاه ناموفق اعلام شده است.",
            "100165" => "افزایش اعتبار کیف پول ریالی کاربر با خطا مواجه شده است.",
            "100166" => "کاهش اعتبار کیف پول ریالی کاربر با خطا مواجه شده است.",
            "100167" => "شناسه تراکنش داخلی معتبر نمیباشد.",
            "100168" => "مبلغ موردنظر خارج از محدوده مجاز میباشد.",
            "100169" => "فراخوانی وب هوک با خظا مواجه شده است.",
            "100170" => "موضوع وب هوک یافت نشد.",
            "100171" => "مقدار سفارش قابل انتقال نمیباشد.",
            "100172" => "مقدار فعلی با مقدار وارد شده یکسان میباشد.",
            "100173" => "محل پرداخت نامعتبر است.",
            "100174" => "درگاه انتخابی با واحد ارزی همخوانی ندارد.",
            "100175" => "کیف پول رمزارزی کاربر یافت نشد.",
            "100176" => "کیف پول متعلق به کاربر نمیباشد.",
            "100177" => "موجودی رمزارزی کاربر کافی نمیباشد",
            "100178" => "موجودی ریالی کاربر کافی نمیباشد.",
            "100179" => "دریافت موجودی رمزارزی کاربر با خطا مواجه شد.",
            "100180" => "خطا در بروزرسانی وضعیت پرداخت.",
            "100181" => "تراکنش متعلق به کاربر مورد نظر نمیباشد.",
            "100182" => "بروزرسانی کیف پول رمزارزی با خطا مواجه شده است.",
            "100183" => "خطا در بروزرسانی شناسه تراکنش خارجی.",
            "100184" => "تراکنش خارجی رمزاری یافت نشد.",
            "100185" => "توضیخات نامعتبر است.",
            "100186" => "شناسه وضعیت نامعتبر است.",
            "100187" => "شناسه تراکنش خارجی نامعتبر است.",
            "100188" => "خطا در بروزرسانی توضیحات",
            "100189" => "خطا در بروزرسانی متادیتا",
            "100190" => "وضعیت پرداختی موجود یافت نشد.",
            "100191" => "خطا در بروزرسانی آدرس مبدا",
            "100192" => "آدرس کیف پول مبدا نامعتبر است.",
            "100193" => "آدرس کیف پول مقصد وارد نشده است.",
            "100194" => "آدرس کیف پول مقصد نامعتبر است.",
            "100195" => "خطا در بروزرسانی آدرس مقصد",
            "100196" => "آدرس کیف پول مبدا وارد نشده است.",
            "100197" => "شناسه تراکنش داخل شبکه‌ای نامعتبر است.",
            "100198" => "آدرس کیف پول مبدا و مقصد نمیتواند یکسان باشد.",
            "100199" => "امکان تغییر آدرس کیف پول برای تراکنش تایید شده وجود ندارد.",
            "100200" => "امکان تغییر شناسه تراکنش داخل شبکه‌ای برای تراکنش تایید شده وجود ندارد.",
            "100201" => "آدرس کیف پول فعلی و ارسال شده یکسان است.",
            "100202" => "شناسه تراکنش داخل شبکه‌ای فعلی و ارسال شده یکسان است.",
            "100203" => "خطا در دریافت شناسه تراکنش داخل شبکه‌ای.",
            "100204" => "خطا در بروزرسانی شناسه یکتا درگاه",
            "100205" => "شناسه یکتا درگاه نامعتبر است.",
            "100206" => "خطا در بروزرسانی شناسه پیگیری درگاه.",
            "100207" => "شناسه پیگیری درگاه نامعتبر است.",
            "100208" => "خطا در تایید \ عدم تایید تراکنش.",
            "100209" => "مقدار تایید \ عدم تایید تراکنش نامعتبر است.",
            "100210" => "مقدار متادیتا نامعتبر است.",
            "100211" => "مقدار تایید \ عدم تایید فعلی و ارسال شده یکسان است.",
            "100212" => "درگاه مستقیم برای واحد مدنظر یافت نشد.",
            "100213" => "ایجاد تراکنش رمزارزی داخلی جدید بابت شارژ کیف پول با خطا مواجه شده است.",
            "100214" => "بروزرسانی تراکنش رمزارزی داخلی بابت شارژ کیف پول با خطا مواجه شده است.",
            "100215" => "ایجاد تراکنش رمزارزی داخلی جدید بابت برداشت از کیف پول با خطا مواجه شده است.",
            "100216" => "بروزرسانی تراکنش رمزارزی داخلی بابت برداشت از کیف پول با خطا مواجه شده است.",
            "100217" => "دریافت وضعیت های مورد نیاز با خطا مواجه شده است.",
            "100218" => "دریافت درگاه های مورد نیاز با خطا مواجه شده است.",
            "100219" => "ایجاد تراکنش رمزارزی خارجی جدید بابت انتقال از کیف پول با خطا مواجه شده است.",
            "100220" => "بروزرسانی تراکنش رمزارزی داخلی بابت ثبت شناسه تراکنش خارجی با خطا مواجه شده است.",
            "100221" => "واحد مورد نظر از نوع رمزارز نمیباشد.",
            "100222" => "امکان ثبت انتقال خارجی برای شبکه رمزارز مورد نظر وجود ندارد.",
            "100223" => "امکان ثبت انتقال خارجی برای واحد رمزارز مورد نظر وجود ندارد.",
            "100224" => "خطا در دریافت موجودی آدرس رمزارزی.",
            "100225" => "آدرس فرستنده مورد تایید شبکه نمیباشد.",
            "100226" => "آدرس گیرنده مورد تایید شبکه نمیباشد.",
            "100227" => "انتقال رمزارز با خطایی روبرو شده است",
            "100228" => "انتقال رمزارز به درستی صورت نپذیرفته است.",
            "100229" => "بروزرسانی موجودی کیف پول داخلی رمزارزی کاربر با خطا مواجه شد.",
            "100230" => "وضعیت سفارش انجام شده یافت نشد.",
            "100231" => "وب‌هوک فعلی و ارسال شده یکسان است.",
            "100232" => "استعلام تراکنش مورد نظر مقدور نمیباشد.",
            "100233" => "تراکنش در وضعیت مجاز جهت استعلام نمیباشد.",
        ];
        $errorMessage = isset($payResponse->data->errorCode) && (array_key_exists($payResponse->data->errorCode,$errorList))
            ? $errorList[$payResponse->data->errorCode]
            : json_encode($payResponse->data);
        sendMessage("خطای سرور  PardakhtSaz:\n\n" .$errorMessage , null, null, $admin);
...
if($data=="pardakhtSazList" && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $keys = json_encode(['inline_keyboard'=>[
        [['text' => $buttonValues['back_button'], 'callback_data' => "managePanel"]]
    ]]);
    editText($message_id,"این قابلیت هنوز فعال نشده",$keys);
}
